...
|---|---|---|---|---|
| **핵심 도메인 데이터** | 음식점 이름/설명, 메뉴 이름/설명, 카테고리 | **등록/수정 시점 (자동 비동기)** | Food Catalog Service → Kafka → Translation Service → AI API | `restaurant_translation`, `menu_translation` DB |
| **사용자 생성 콘텐츠 (UGC)** | 리뷰, 문의사항, 사용자 커뮤니티 | **사용자 요청 시점 (온디맨드)** | Frontend [번역] 버튼 클릭 → Translation Service → AI API | Redis Cache (TTL) + `ugc_translation` DB |
...
Food Catalog Service
        │
        │ 1. RestaurantCreated / MenuCreated Event
        ▼
   ...
   Translation Service (Consumer)
        │
        ├─► 2. 중복 번역 검사 (Redis/DB Lookup)
        │
        ├─► 3. AI API 호출 (OpenAI / DeepL / Gemini 등)
        │
        └─► 4. 번역 결과 DB 저장 (`restaurant_translation`, `menu_translation`)
                │
                └─► 5. Kafka Topic: `translation-results` (필요시 Food Catalog 알림)
```

### 흐름 상세

1. **이벤트 발행**: 점주가 한국어로 음식점/메뉴를 등록하면 `food-catalog-service`에서 Kafka 이벤트가 발행됩니다.
2. **비동기 처리**: `translation-service`가 이벤트를 소비하고 지원 언어(`ja` 등)로 AI 번역을 수행합니다.
3. **영속화**: 번역된 결과는 `restaurant_translation`, `menu_translation` 테이블에 즉시 저장되어 향후 조회가 빠르게 이루어집니다.
